...
bool Direct3D::Initialize(HWND__* windowHandle, int windowWidth, int windowHeight)
{
...
{
	viewPorts[0].Width = (FLOAT)m_windowWidth;
	viewPorts[0].Height = (FLOAT)m_windowHeight;
...
bool Direct3D::InitializeShadowDepthBufferAndDepthStencilView()
{
	...
	depthBufferDesc.Width = m_windowWidth;
	depthBufferDesc.Height = m_windowHeight;	// Change to quadratic later ?
	depthBufferDesc.MipLevels = 1;
	depthBufferDesc.ArraySize = 1;
...
{
	viewPorts[1].Width = (float)m_windowWidth;
	viewPorts[1].Height = (float)m_windowHeight;
