...
#include "packages/directxtk_desktop_2015.2018.10.31.1/include/SpriteFont.h"
#include "packages/directxtk_desktop_2015.2018.10.31.1/include/SpriteBatch.h"
...
{
	unsigned int nTexts = static_cast<unsigned int>(m_texts.size());
	unsigned int nImages = static_cast<unsigned int>(m_images.size());
	...
	*/
	for (unsigned int i = 0; i < nTexts; i++)
	{
		...
		std::wstring wstr = StringConverter::ToWideString(text->text);
...
{
	return GetDimensionsOfText(StringConverter::ToWideString(text));
}
...
{
	unsigned int n = static_cast<unsigned int>(m_texts.size());
...
{
	unsigned int n = static_cast<unsigned int>(m_images.size());
	for (unsigned int i = 0; i < n; i++)
	{
		if (m_images[i].id == id)
		{
