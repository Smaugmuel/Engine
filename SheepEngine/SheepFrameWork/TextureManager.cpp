...
#include "packages/directxtk_desktop_2015.2018.10.31.1/include/WICTextureLoader.h"
...
int TextureManager::LoadTexture(const std::string & fileName)
{
	...
	*/
	std::wstring wstr = StringConverter::ToWideString("../Textures/" + std::string(fileName));
	if (wstr.size() == 0)
		return -1;
	...
	*/
	...
	if (FAILED(DirectX::CreateWICTextureFromFile(m_device, wstr.c_str(), nullptr, &resource)))
		return -1;
