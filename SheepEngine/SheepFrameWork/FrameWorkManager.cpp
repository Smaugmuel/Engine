...
#endif
{
	...
	if (!m_input.Initialize(m_window.GetHandle()))
		return false;
	if (!m_d3d.Initialize(m_window.GetHandle(), windowSize.x, windowSize.y))
		return false;
	...
	if (!m_samplerManager.Initialize(m_d3d.GetDevice(), m_d3d.GetDeviceContext()))
		return false;
	if (!m_deferredRenderingManager.Initialize(m_d3d.GetDevice(), m_d3d.GetDeviceContext(), windowSize.x, windowSize.y))
		return false;
